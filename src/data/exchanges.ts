import { Exchange } from "@/types/exchange";

export const exchanges: Exchange[] = [
  // ========== 国内取引所 ==========
  {
    id: "bitflyer",
    name: "bitFlyer",
    nameEn: "bitFlyer",
    region: "domestic",
    country: "日本",
    established: 2014,
    url: "https://bitflyer.com/ja-jp/",
    domain: "bitflyer.com",
    logoFile: "bitflyer.webp",
    logoColor: "#1652F0",
    description:
      "国内最大級のビットコイン取引量を誇る老舗取引所。金融庁登録済みで高い信頼性を持つ。Lightning FXなどのデリバティブ取引も可能。",
    tokens: [
      "BTC", "ETH", "XRP", "LTC", "BCH", "ETC", "XEM", "XLM", "MONA", "LSK",
      "BAT", "XTZ", "DOT", "XYM", "LINK", "MATIC", "POL", "DOGE", "MKR",
      "FLR", "SAND", "SHIB", "AXS", "MANA", "IMX", "CHZ", "APE", "DAI",
      "AVAX", "RNDR", "GRT", "MASK", "PEPE",
    ],
    features: [
      "spot",
      "futures",
      "margin",
      "api",
      "mobile_app",
      "japan_yen_deposit",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.01,
      exchangeTaker: 0.15,
      dealerSpread: 5.0,
      roundTripCostPct: 5.0,
      spreadRating: "very_wide",
      depositFeeNote: "住信SBIネット銀行から無料、その他銀行330円〜",
      withdrawalFeeNote: "220円〜440円",
    },
    minTradeAmountJPY: 1,
    maxLeverage: 2,
    tradingPairs: 15,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes: "重大なセキュリティ事故なし。取引所(BTC/JPY)は国内最高峰の流動性。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "A8.net / ValueCommerce / もしもアフィリエイト 等で案件あり",
    travelRule: { solution: "TRUST",       note: "2023年5月30日対応。TRUSTネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "coincheck",
    name: "Coincheck",
    nameEn: "Coincheck",
    region: "domestic",
    country: "日本",
    established: 2012,
    url: "https://coincheck.com/ja/",
    domain: "coincheck.com",
    logoFile: "coincheck.png",
    logoColor: "#00B7A4",
    description:
      "国内最多水準の取扱銘柄数を誇る取引所。NFTマーケットプレイスや積立投資など多彩なサービスを提供。マネックスグループ傘下。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "BCH", "ETC",
      "XEM", "XLM", "MONA", "LSK", "IOST", "QTUM", "ENJ", "SAND", "CHZ",
      "LINK", "DAI", "AVAX", "SHIB", "MKR", "APE", "RNDR", "PEPE", "POL",
      "AXS", "XYM", "OMG", "BAT", "IMX",
    ],
    features: [
      "spot",
      "staking",
      "lending",
      "nft",
      "api",
      "mobile_app",
      "japan_yen_deposit",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.0,
      exchangeTaker: 0.0,
      dealerSpread: 1.5,
      roundTripCostPct: 1.5,
      spreadRating: "wide",
      depositFeeNote: "銀行振込無料、コンビニ入金756円〜",
      withdrawalFeeNote: "407円",
    },
    minTradeAmountJPY: 500,
    tradingPairs: 30,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 2,
    notes: "2018年1月にNEM不正流出事故（約580億円相当）が発生。全額補償を実施し現在は金融庁登録済みで運営継続中。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "ValueCommerce / A8.net / afb 等で案件あり。成果報酬は口座開設完了",
    travelRule: { solution: "TRUST",       note: "2023年5月31日対応。TRUSTネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "gmo-coin",
    name: "GMOコイン",
    nameEn: "GMO Coin",
    region: "domestic",
    country: "日本",
    established: 2016,
    url: "https://coin.z.com/jp/",
    domain: "coin.z.com",
    logoFile: "gmocoin.png",
    logoColor: "#009BDE",
    description:
      "GMOインターネットグループ傘下の取引所。業界最安水準の手数料と豊富な取引機能が特徴。取引所での売買手数料がマイナス(リベート)になることも。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "BCH",
      "ATOM", "LINK", "XLM", "XEM", "MONA", "XTZ", "DAI", "MKR", "FIL",
      "SAND", "CHZ", "AVAX", "ETC", "BAT", "SHIB", "XYM", "AXS",
    ],
    features: [
      "spot",
      "futures",
      "margin",
      "staking",
      "api",
      "mobile_app",
      "japan_yen_deposit",
    ],
    fees: {
      exchangeMaker: -0.01,
      exchangeTaker: 0.05,
      dealerSpread: 0.5,
      roundTripCostPct: 0.12,
      spreadRating: "tight",
      depositFeeNote: "無料",
      withdrawalFeeNote: "無料",
    },
    minTradeAmountJPY: 1,
    maxLeverage: 2,
    tradingPairs: 26,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes: "重大なセキュリティ事故なし。入出金手数料が完全無料で実質コストが低い。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "GMOアフィリエイト / A8.net で案件あり",
    travelRule: { solution: "Sygna",       note: "2023年5月31日対応。Sygnaネットワーク参加取引所への送金に対応。詳細は各取引所の公式サイトでご確認ください" },
  },
  {
    id: "bitbank",
    name: "bitbank",
    nameEn: "bitbank",
    region: "domestic",
    country: "日本",
    established: 2014,
    url: "https://bitbank.cc/",
    domain: "bitbank.cc",
    logoFile: "bitbank.png",
    logoColor: "#2B5FEC",
    description:
      "国内でも有数の取引量を誇るプロ向け取引所。チャートや板が充実しており本格的なトレードが可能。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "BCH",
      "LINK", "XLM", "MONA", "BAT", "QTUM", "OMG", "XYM", "ASTR",
      "AVAX", "SAND", "GRT", "RNDR", "BNB", "ARB", "OP", "DAI", "IMX",
      "MASK", "SHIB", "ETC", "AXS",
    ],
    features: ["spot", "api", "mobile_app", "japan_yen_deposit"],
    fees: {
      exchangeMaker: -0.02,
      exchangeTaker: 0.12,
      dealerSpread: 0.5,
      roundTripCostPct: 0.24,
      spreadRating: "tight",
      depositFeeNote: "無料",
      withdrawalFeeNote: "550円〜770円",
    },
    minTradeAmountJPY: 1,
    tradingPairs: 30,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes: "重大なセキュリティ事故なし。取引板が充実しており国内で本格的なトレードに最適。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "A8.net / もしもアフィリエイト 等で案件あり",
    travelRule: { solution: "Sygna",       note: "2023年6月9日対応。Sygnaネットワーク参加取引所への送金に対応。詳細は各取引所の公式サイトでご確認ください" },
  },
  {
    id: "sbi-vc-trade",
    name: "SBI VCトレード",
    nameEn: "SBI VC Trade",
    region: "domestic",
    country: "日本",
    established: 2018,
    url: "https://www.sbivc.co.jp/",
    domain: "sbivc.co.jp",
    logoFile: "sbivctrade.png",
    logoColor: "#E4001B",
    description:
      "SBIホールディングス傘下の取引所。高い信頼性と充実したサービスが特徴。SBIグループとの連携が強み。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "BCH",
      "LINK", "XLM", "FLARE", "FLR", "ATOM", "DAI", "USDC", "HBAR",
      "AVAX", "SHIB", "APT", "NEAR", "ALGO", "AXS", "BAT", "CHZ", "ENJ",
      "ETC", "MKR", "MONA", "OMG", "SAND", "TRX",
    ],
    features: [
      "spot",
      "futures",
      "margin",
      "staking",
      "api",
      "mobile_app",
      "japan_yen_deposit",
    ],
    fees: {
      exchangeMaker: 0.0,
      exchangeTaker: 0.0,
      dealerSpread: 0.5,
      roundTripCostPct: 0.1,
      spreadRating: "tight",
      depositFeeNote: "無料",
      withdrawalFeeNote: "無料",
    },
    minTradeAmountJPY: 1,
    maxLeverage: 2,
    tradingPairs: 18,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes: "重大なセキュリティ事故なし。取引手数料・入出金手数料すべて無料。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "A8.net / ValueCommerce 等で案件あり",
    travelRule: { solution: "Sygna+TRUST", note: "2024年4月24日よりTRUSTも追加。両ソリューションに対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "bybit-jp",
    name: "Bybit Japan",
    nameEn: "Bybit Japan",
    region: "domestic",
    country: "日本",
    established: 2024,
    url: "https://www.bybit.com/ja-JP/",
    domain: "bybit.com",
    logoFile: "Bybit.png",
    logoColor: "#F7A600",
    description:
      "海外大手Bybitの日本法人。金融庁登録を取得し国内サービスを提供。豊富な銘柄と低い手数料が魅力。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "SOL",
      "ADA",
      "DOT",
      "DOGE",
      "AVAX",
      "LINK",
      "ATOM",
      "USDT",
      "USDC",
    ],
    features: [
      "spot",
      "staking",
      "api",
      "mobile_app",
      "japan_yen_deposit",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.22,
      spreadRating: "tight",
      depositFeeNote: "無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 40,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 3,
    notes: "国内法人は別法人だが、親会社Bybitが2025年2月に約2,200億円相当のハッキング被害を受けた。",
    affiliateUrl: "https://partner.bybit.com/b/118679",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録で手数料割引特典あり",
    travelRule: { solution: "不明",        note: "Bybitグローバルはトラベルルール対応済みだが、日本法人の詳細は未公表" },
  },
  {
    id: "rakuten-wallet",
    name: "楽天ウォレット",
    nameEn: "Rakuten Wallet",
    region: "domestic",
    country: "日本",
    established: 2019,
    url: "https://wallet.rakuten.co.jp/",
    domain: "rakuten.co.jp",
    logoFile: "rakutenwallet.png",
    logoColor: "#BF0000",
    description:
      "楽天グループの暗号資産サービス。楽天銀行からの即時入金や楽天ポイント連携が特徴。シンプルなUIで初心者にも使いやすい。",
    tokens: ["BTC", "ETH", "BCH", "SOL", "ADA", "AVAX", "DOT", "MATIC"],
    features: ["spot", "mobile_app", "japan_yen_deposit", "credit_card"],
    fees: {
      dealerSpread: 3.0,
      roundTripCostPct: 3.0,
      spreadRating: "wide",
      depositFeeNote: "楽天銀行から即時入金無料、他銀行振込手数料あり",
      withdrawalFeeNote: "330円〜",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 9,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes: "重大なセキュリティ事故なし。楽天グループが運営し信頼性が高い。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "楽天アフィリエイトや各種ASPで案件あり",
    travelRule: { solution: "Sygna",       note: "JVCEA加盟。Sygnaネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "sblox",
    name: "S.BLOX",
    nameEn: "S.BLOX",
    region: "domestic",
    country: "日本",
    established: 2019,
    url: "https://www.s-blox.com/",
    domain: "s-blox.com",
    logoFile: "sblox.svg",
    logoColor: "#1A1F35",
    description:
      "ソニーグループ傘下の暗号資産サービス。2025年1月に旧「WhaleFin」からリニューアル。ソニー独自ブロックチェーン「Soneium」との連携も展開中。",
    tokens: ["BTC", "ETH", "BCH", "LTC", "XRP"],
    features: ["spot", "mobile_app", "japan_yen_deposit"],
    fees: {
      dealerSpread: 3.0,
      roundTripCostPct: 3.0,
      spreadRating: "wide",
      depositFeeNote: "銀行振込対応",
      withdrawalFeeNote: "330円/回",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 6,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 4,
    notes:
      "重大なセキュリティ事故なし。2025年1月に旧WhaleFin（アンバージャパン）からS.BLOXとしてリニューアル。ソニーグループ傘下。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "各種ASPで案件あり",
    travelRule: { solution: "Sygna",       note: "JVCEA加盟。Sygnaネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "bittrade",
    name: "ビットトレード",
    nameEn: "BitTrade",
    region: "domestic",
    country: "日本",
    established: 2017,
    url: "https://bittrade.co.jp/",
    domain: "bittrade.co.jp",
    logoFile: "bittrade.png",
    logoColor: "#1B5FAA",
    description:
      "HTX（旧Huobi）グループの日本法人。国内最多水準の取扱銘柄数が強み。取引所・販売所の両方を提供し、多彩なアルトコインが取引可能。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "LTC",
      "BCH",
      "XLM",
      "ETC",
      "BAT",
      "ADA",
      "DOT",
      "IOST",
      "BNB",
      "DOGE",
      "SOL",
      "SAND",
      "MATIC",
      "ATOM",
      "ENJ",
    ],
    features: ["spot", "mobile_app", "japan_yen_deposit", "api"],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      dealerSpread: 3.5,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "銀行振込対応",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 500,
    tradingPairs: 50,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 4,
    notes:
      "重大なセキュリティ事故なし。HTXグループ（旧Huobi）の日本法人。国内最多水準の取扱銘柄数が特徴。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "各種ASPで案件あり",
    travelRule: { solution: "Sygna",       note: "JVCEA加盟。Sygnaネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "mercoin",
    name: "メルコイン",
    nameEn: "Mercoin",
    region: "domestic",
    country: "日本",
    established: 2022,
    url: "https://mercoin.jp/",
    domain: "mercoin.jp",
    logoFile: "mercoin_logo.svg",
    logoColor: "#FF0211",
    description:
      "メルカリグループの暗号資産サービス。メルペイ残高での購入や自動積立が可能。BTC・ETH・XRPに特化したシンプル設計で初心者向け。",
    tokens: ["BTC", "ETH", "XRP"],
    features: ["spot", "mobile_app", "japan_yen_deposit"],
    fees: {
      dealerSpread: 3.5,
      roundTripCostPct: 3.5,
      spreadRating: "wide",
      depositFeeNote: "メルペイ残高から即時入金可、銀行振込対応",
      withdrawalFeeNote: "200円",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 3,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 5,
    notes:
      "重大なセキュリティ事故なし。メルカリグループの信頼性とメルペイ連携が強み。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "各種ASPで案件あり",
    travelRule: { solution: "不明",        note: "比較的新しい取引所のため詳細情報未確認" },
  },
  {
    id: "bitpoint",
    name: "ビットポイント",
    nameEn: "BitPoint",
    region: "domestic",
    country: "日本",
    established: 2016,
    url: "https://www.bitpoint.co.jp/",
    domain: "bitpoint.co.jp",
    logoFile: "bitpoint.svg",
    logoColor: "#0066CC",
    description:
      "SBIグループ傘下の暗号資産取引所。取引所手数料が完全無料で取引コストを抑えられる。多様なアルトコインに対応し銘柄数も豊富。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "LTC",
      "BCH",
      "BAT",
      "ADA",
      "DOT",
      "LINK",
      "IOST",
      "MATIC",
      "ATOM",
      "SOL",
      "DOGE",
      "AVAX",
      "BNB",
      "XLM",
      "FLARE",
    ],
    features: ["spot", "mobile_app", "japan_yen_deposit", "api"],
    fees: {
      exchangeMaker: 0.0,
      exchangeTaker: 0.0,
      dealerSpread: 3.5,
      roundTripCostPct: 0.01,
      spreadRating: "tight",
      depositFeeNote: "銀行振込・即時入金無料",
      withdrawalFeeNote: "330円（月1回無料）",
    },
    minTradeAmountJPY: 500,
    tradingPairs: 27,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 2,
    notes:
      "2019年7月にハッキング被害（約35億円相当）が発生。全額補償を実施し現在も運営継続中。SBIグループ傘下で体制強化済み。",
    affiliateUrl: "",
    affiliateType: "asp",
    affiliateNote: "各種ASPで案件あり",
    travelRule: { solution: "Sygna+TRUST", note: "2024年1月18日よりTRUSTも追加。両ソリューションに対応。詳細は公式サイトでご確認ください" },
  },

  // ========== 海外ブランド日本法人・海外取引所 ==========
  {
    id: "okx-japan",
    name: "OKX Japan",
    nameEn: "OKX Japan",
    region: "domestic",
    country: "日本",
    established: 2017,
    url: "https://www.okx.com/ja",
    domain: "okx.com",
    logoFile: "OKX.png",
    logoColor: "#121212",
    description:
      "世界大手OKXの日本法人。2017年にOKCoin Japanとして設立後、OKX Japanとしてサービスを提供。金融庁登録済みで安心して利用可能。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "SOL",
      "ADA",
      "DOT",
      "MATIC",
      "DOGE",
      "LTC",
      "AVAX",
      "LINK",
      "UNI",
      "ATOM",
      "USDT",
      "USDC",
    ],
    features: [
      "spot",
      "staking",
      "api",
      "mobile_app",
      "japan_yen_deposit",
    ],
    fees: {
      exchangeMaker: 0.08,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.19,
      spreadRating: "tight",
      depositFeeNote: "銀行振込対応",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 500,
    tradingPairs: 30,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 4,
    notes: "重大なセキュリティ事故なし。2017年設立の老舗日本法人。金融庁登録済み。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラム: okx.com/ja/affiliate で発行",
    tokensUrl: "https://www.okx.com/ja/markets/prices",
    travelRule: { solution: "不明",        note: "OKX JapanのJVCEA加盟に基づく対応状況は未確認" },
  },
  {
    id: "bitget",
    name: "Bitget",
    nameEn: "Bitget",
    region: "overseas",
    country: "シンガポール",
    established: 2018,
    url: "https://www.bitget.com/ja",
    domain: "bitget.com",
    logoFile: "bitget.png",
    logoColor: "#00C096",
    description:
      "コピートレード機能で急成長した取引所。豊富な取引ペアとリーズナブルな手数料が魅力。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "SOL",
      "ADA",
      "DOT",
      "MATIC",
      "DOGE",
      "LTC",
      "AVAX",
      "LINK",
      "UNI",
      "ATOM",
      "USDT",
      "USDC",
    ],
    features: [
      "spot",
      "futures",
      "margin",
      "staking",
      "copy_trading",
      "demo",
      "api",
      "mobile_app",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 125,
    tradingPairs: 800,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 6,
    notes: "重大なセキュリティ事故なし。コピートレード機能が業界トップレベル。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式アフィリエイトプログラム: bitget.com/ja/affiliate で発行",
    tokensUrl: "https://www.bitget.com/ja/spot",
    travelRule: { solution: "TRUST",       note: "TRUSTネットワーク参加取引所への送金に対応。詳細は公式サイトでご確認ください" },
  },
  {
    id: "mexc",
    name: "MEXC",
    nameEn: "MEXC",
    region: "overseas",
    country: "セーシェル",
    established: 2018,
    url: "https://www.mexc.com/ja-JP",
    domain: "mexc.com",
    logoFile: "mexc.svg",
    logoColor: "#1B49E8",
    description:
      "新興アルトコインの上場が早い取引所。現物取引のmaker手数料が0%で非常にコストが低い。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "SOL",
      "ADA",
      "DOT",
      "MATIC",
      "DOGE",
      "LTC",
      "AVAX",
      "LINK",
      "UNI",
      "ATOM",
      "USDT",
      "USDC",
    ],
    features: [
      "spot",
      "futures",
      "margin",
      "staking",
      "copy_trading",
      "api",
      "mobile_app",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.0,
      exchangeTaker: 0.0,
      roundTripCostPct: 0.05,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 200,
    tradingPairs: 2000,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 3,
    volumeRank: 9,
    notes: "重大事故なし。ただし主要国での規制登録なし。現物スポット手数料0%、新規アルトコインの取扱いが早い。",
    affiliateUrl: "https://www.mexc.com/ja-JP/acquisition/custom-sign-up?shareCode=mexc-15SZe",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録で手数料割引特典あり",
    tokensUrl: "https://www.mexc.com/ja-JP/markets",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  {
    id: "zoomex",
    name: "Zoomex",
    nameEn: "Zoomex",
    region: "overseas",
    country: "シンガポール",
    established: 2021,
    url: "https://www.zoomex.com/ja-JP/",
    domain: "zoomex.com",
    logoFile: "zoomex.png",
    logoColor: "#1A56DB",
    description:
      "日本人ユーザー向けに特化した海外取引所。サイト・アプリ・カスタマーサポートが完全日本語対応で、入出金手数料が無料。590種類以上の仮想通貨を最大150倍のレバレッジで取引可能。",
    tokens: ["BTC", "ETH", "XRP", "SOL", "ADA", "DOGE", "AVAX", "DOT", "LINK", "MATIC", "LTC", "ATOM", "USDT"],
    features: [
      "spot",
      "futures",
      "margin",
      "copy_trading",
      "demo",
      "api",
      "mobile_app",
    ],
    fees: {
      exchangeMaker: 0.02,
      exchangeTaker: 0.06,
      roundTripCostPct: 0.16,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "無料",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 150,
    tradingPairs: 590,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 3,
    notes: "2021年設立と比較的新しいが、日本人向けサービスに特化。重大なセキュリティ事故は確認されていない。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムあり",
    tokensUrl: "https://www.zoomex.com/ja-JP/trade/spot",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  {
    id: "kucoin",
    name: "KuCoin",
    nameEn: "KuCoin",
    region: "overseas",
    country: "セーシェル",
    established: 2017,
    url: "https://www.kucoin.com/ja",
    domain: "kucoin.com",
    logoFile: "kucoin.png",
    logoColor: "#23AF91",
    description:
      "「The People's Exchange」として知られる大手取引所。700以上のトークンを取扱い、ステーキング・レンディング・コピートレードなど多様な投資手法を提供。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "LTC", "BNB", "AVAX", "LINK", "UNI", "ATOM", "USDT", "USDC",
    ],
    features: [
      "spot", "futures", "margin", "staking", "lending",
      "copy_trading", "nft", "api", "mobile_app", "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄・ネットワークにより異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 100,
    tradingPairs: 700,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 2,
    volumeRank: 8,
    notes:
      "2020年9月にハッキング被害（約420億円相当）が発生。84%を回収し全額補償を実施。現在も運営継続中。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラム: kucoin.com/ja/ucenter/referral で発行",
    tokensUrl: "https://www.kucoin.com/ja/markets",
    travelRule: { solution: "独自対応",    note: "2025年2月24日より全入出金に必須。KuCoin独自システムで対応" },
  },
  {
    id: "crypto-com",
    name: "Crypto.com",
    nameEn: "Crypto.com",
    region: "overseas",
    country: "シンガポール",
    established: 2016,
    url: "https://crypto.com/ja/",
    domain: "crypto.com",
    logoFile: "crypto_com.png",
    logoColor: "#002D74",
    description:
      "世界8,000万人以上が利用する総合型暗号資産プラットフォーム。VISAカード発行など独自機能も充実。7.5億ドルの保険カバレッジを保有する高セキュリティ体制。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "LTC", "AVAX", "LINK", "UNI", "ATOM", "USDT", "USDC",
    ],
    features: [
      "spot", "futures", "staking", "lending", "nft",
      "api", "mobile_app", "credit_card",
    ],
    fees: {
      exchangeMaker: 0.4,
      exchangeTaker: 0.4,
      roundTripCostPct: 0.81,
      spreadRating: "moderate",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 10,
    tradingPairs: 250,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 2,
    volumeRank: 11,
    notes:
      "2022年1月に不正アクセス事案（約50億円相当）が発生。全額補償を実施し継続運営中。7.5億ドルの保険カバレッジ保有。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラム: crypto.com/ja/referral で発行",
    tokensUrl: "https://crypto.com/ja/exchange/markets",
    travelRule: { solution: "独自対応",    note: "複数国の規制当局に登録済み。Notabeneなど独自システムで対応" },
  },
  {
    id: "bingx",
    name: "BingX",
    nameEn: "BingX",
    region: "overseas",
    country: "シンガポール",
    established: 2018,
    url: "https://bingx.com/ja-jp/",
    domain: "bingx.com",
    logoFile: "bingx.png",
    logoColor: "#1677FF",
    description:
      "コピートレード機能に定評のある取引所。初心者がプロトレーダーの取引を自動コピーできる機能が人気。現物・先物・コピートレードを一括で提供。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "LTC", "BNB", "AVAX", "LINK", "UNI", "ATOM", "USDT", "USDC",
    ],
    features: [
      "spot", "futures", "margin", "copy_trading", "staking",
      "api", "mobile_app", "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄・ネットワークにより異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 150,
    tradingPairs: 700,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 2,
    volumeRank: 13,
    notes:
      "2024年9月にハッキング被害（約65億円相当）が発生。補償対応済みで運営継続中。コピートレード機能は業界トップクラス。",
    affiliateUrl: "https://bingxdao.com/invite/MWZMBT/",
    affiliateType: "referral",
    affiliateNote: "",
    tokensUrl: "https://bingx.com/ja-jp/spot/",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  {
    id: "phemex",
    name: "Phemex",
    nameEn: "Phemex",
    region: "overseas",
    country: "シンガポール",
    established: 2019,
    url: "https://phemex.com/ja",
    domain: "phemex.com",
    logoFile: "phemex.png",
    logoColor: "#21B7FF",
    description:
      "元Morgan Stanley幹部が設立したプロ向け取引所。高速マッチングエンジンと豊富なデリバティブが特徴。デモトレード機能で初心者も練習しながら取引できる。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "LTC", "AVAX", "LINK", "UNI", "ATOM", "USDT", "USDC",
    ],
    features: [
      "spot", "futures", "margin", "staking", "lending",
      "demo", "api", "mobile_app", "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄・ネットワークにより異なる",
    },
    minTradeAmountJPY: 100,
    maxLeverage: 100,
    tradingPairs: 300,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 2,
    volumeRank: 45,
    notes:
      "2025年1月にハッキング被害（約110億円相当）が発生（Lazarusグループ関与疑い）。補償対応中で運営継続中。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラム: phemex.com/ja/user-center/referral で発行",
    tokensUrl: "https://phemex.com/ja/spot-trading",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  {
    id: "woox",
    name: "WOO X",
    nameEn: "WOO X",
    region: "overseas",
    country: "ケイマン諸島",
    established: 2019,
    url: "https://www.woox.io/ja-JP",
    domain: "woox.io",
    logoFile: "woox.png",
    logoColor: "#0BC8F1",
    description:
      "機関投資家向け流動性プロバイダーWootradeが運営。WOOトークンのステーキングで取引手数料を大幅削減（最大0%）できる独自モデルが特徴。深い流動性とタイトなスプレッドが強み。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "AVAX", "LINK", "UNI", "ATOM", "USDT", "USDC",
    ],
    features: ["spot", "futures", "margin", "staking", "api", "mobile_app"],
    fees: {
      exchangeMaker: 0.02,
      exchangeTaker: 0.05,
      roundTripCostPct: 0.08,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "WOOステーキングで無料",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 200,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 50,
    notes:
      "重大なセキュリティ事故なし。機関投資家向け流動性プロバイダーのWootradeが母体。WOOトークンのステーキングで取引手数料の0%化が可能。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムで発行可能",
    tokensUrl: "https://www.woox.io/ja-JP/markets",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  {
    id: "hashkey-global",
    name: "HashKey Global",
    nameEn: "HashKey Global",
    region: "overseas",
    country: "バミューダ",
    established: 2024,
    url: "https://global.hashkey.com/",
    domain: "hashkey.com",
    logoFile: "HashKey_Global.png",
    logoColor: "#1B44D8",
    description:
      "香港SFC・バミューダBMAのダブルライセンスを取得した高規制準拠取引所。HashKeyグループは日本でも東京ハッシュが金融庁登録済み。透明性と安全性を最優先とする機関投資家向けプラットフォーム。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "AVAX",
      "LINK", "ATOM", "USDT", "USDC",
    ],
    features: ["spot", "futures", "staking", "api", "mobile_app"],
    fees: {
      exchangeMaker: 0.06,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.17,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 100,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 5,
    volumeRank: 65,
    notes:
      "重大なセキュリティ事故なし。香港SFC・バミューダBMAの規制ライセンス保有。HashKeyグループは日本でも東京ハッシュ株式会社（金融庁登録第00027号）を展開。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムで発行可能",
    tokensUrl: "https://global.hashkey.com/en-US/markets",
    travelRule: { solution: "独自対応",    note: "香港SFC（証券先物委員会）規制準拠。アジア最大級の規制対応取引所" },
  },
  {
    id: "bitrue",
    name: "Bitrue",
    nameEn: "Bitrue",
    region: "overseas",
    country: "シンガポール",
    established: 2018,
    url: "https://www.bitrue.com/ja/",
    domain: "bitrue.com",
    logoFile: "bitrue.png",
    logoColor: "#1B4FEB",
    description:
      "XRP特化の取引所として知られ、XRPペアの取扱量は業界トップクラス。フレキシブルXRPステーキングやPower Piggyなど独自の利回り商品が充実。XRP保有者に最適。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "MATIC", "DOGE",
      "LTC", "AVAX", "LINK", "ATOM", "USDT", "USDC",
    ],
    features: ["spot", "staking", "lending", "api", "mobile_app", "credit_card"],
    fees: {
      exchangeMaker: 0.098,
      exchangeTaker: 0.0686,
      roundTripCostPct: 0.18,
      spreadRating: "tight",
      depositFeeNote: "暗号資産送金無料",
      withdrawalFeeNote: "XRP出金無料、他銘柄は銘柄により異なる",
    },
    minTradeAmountJPY: 100,
    tradingPairs: 500,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 2,
    volumeRank: 80,
    notes:
      "2023年4月にハッキング被害（約35億円相当）が発生。全額補償を実施し継続運営中。XRPペアの流動性と利回り商品が業界トップクラス。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムで発行可能",
    tokensUrl: "https://www.bitrue.com/ja/trade/",
    travelRule: { solution: "不明",        note: "公式のトラベルルール対応状況は未公表" },
  },
  // ========== 海外ブランドの日本法人 ==========
  {
    id: "binance-japan",
    name: "Binance Japan",
    nameEn: "Binance Japan",
    region: "domestic",
    country: "日本",
    established: 2023,
    url: "https://www.binance.com/ja-JP",
    domain: "binance.com",
    logoFile: "binance.png",
    logoColor: "#F0B90B",
    description:
      "世界最大手バイナンスの日本法人。バイナンスジャパン株式会社として金融庁登録済み。豊富な銘柄数と高い流動性が特徴。",
    tokens: [
      "BTC",
      "ETH",
      "XRP",
      "SOL",
      "ADA",
      "DOT",
      "MATIC",
      "DOGE",
      "LTC",
      "BNB",
      "AVAX",
      "LINK",
      "UNI",
      "ATOM",
      "USDT",
      "USDC",
    ],
    features: [
      "spot",
      "staking",
      "api",
      "mobile_app",
      "japan_yen_deposit",
      "credit_card",
    ],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.21,
      spreadRating: "tight",
      depositFeeNote: "銀行振込・コンビニ入金対応",
      withdrawalFeeNote: "銘柄により異なる",
    },
    minTradeAmountJPY: 500,
    tradingPairs: 50,
    japaneseSupport: true,
    fsaRegistered: true,
    trustScore: 4,
    notes:
      "2023年日本市場参入。金融庁登録済みで国内規制に準拠。親会社はグローバルNo.1取引所のBinance。",
    affiliateUrl: "https://s.binance.com/i5hlAdtO?ref=GRO_40299_LLEQA",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録で手数料割引特典あり",
    tokensUrl: "https://www.binance.com/ja-JP/markets/overview",
    travelRule: { solution: "不明",        note: "Binance Japanの国内向けトラベルルール対応の詳細は未公表" },
  },

  // ========== その他の海外取引所 ==========
  {
    id: "orangex",
    name: "OrangeX",
    nameEn: "OrangeX",
    region: "overseas",
    country: "チェコ",
    established: 2021,
    url: "https://www.orangex.com/ja-jp/",
    domain: "orangex.com",
    logoFile: "orangex.svg",
    logoColor: "#FF6B00",
    description:
      "2021年設立の新興取引所。チェコにてVASPライセンス取得済み、米FinCEN登録のMSBも保有。完全日本語対応・24時間日本語サポート。コピートレード機能が充実し、Bybit乗り換え先として日本で急速に注目を集めている。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "AVAX",
      "LINK", "ATOM", "MATIC", "USDT", "USDC",
    ],
    features: ["spot", "futures", "copy_trading", "api", "mobile_app"],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.16,
      spreadRating: "tight",
      depositFeeNote: "入金手数料無料",
      withdrawalFeeNote: "銘柄により異なる（BTC: 0.00025 BTC等）",
    },
    maxLeverage: 200,
    tradingPairs: 545,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 3,
    notes:
      "2023年9月に1,000万ドルの初回VC資金調達、2025年には2,000万ドルの第2ラウンドを完了。1,000 BTCの保険基金を保有。LexisNexis Risk Solutions・SlowMistとのコンプライアンス連携。金融庁警告なし（2025年時点）。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムで発行可能",
    tokensUrl: "https://www.orangex.com/ja-jp/trade/",
    travelRule: { solution: "独自対応", note: "2024年12月30日にVerifyVASP提供のTravel Rule APIを統合完了。EU MiCA規制・FATF指針準拠" },
  },
  {
    id: "btcc",
    name: "BTCC",
    nameEn: "BTCC",
    region: "overseas",
    country: "英国",
    established: 2011,
    url: "https://www.btcc.com/ja-JP/",
    domain: "btcc.com",
    logoFile: "btcc.png",
    logoColor: "#E8B20A",
    description:
      "2011年設立、世界で最も歴史ある仮想通貨取引所のひとつ。設立以来14年間セキュリティ事故ゼロを誇る。米FinCEN・カナダFINTRAC・欧州（リトアニア）の3か国でライセンス取得済み。ゼロカット制度で追証なし。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC",
      "AVAX", "LINK", "ATOM", "USDT", "USDC",
    ],
    features: ["spot", "futures", "copy_trading", "demo", "api", "mobile_app", "credit_card"],
    fees: {
      exchangeMaker: 0.06,
      exchangeTaker: 0.06,
      roundTripCostPct: 0.12,
      spreadRating: "tight",
      depositFeeNote: "入金手数料無料",
      withdrawalFeeNote: "銘柄により異なる",
    },
    maxLeverage: 225,
    tradingPairs: 850,
    japaneseSupport: true,
    fsaRegistered: false,
    trustScore: 4,
    notes:
      "2011年創業の最古参取引所。設立以来ハッキング被害ゼロ。ゼロカット制度（追証なし）採用。VIPプログラムで手数料最安0.01%まで低減可。デモ取引口座あり。金融庁警告なし（2025年時点）。",
    affiliateUrl: "https://btcc.com/invite/Uf27pnl",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録でボーナスあり",
    tokensUrl: "https://www.btcc.com/ja-JP/markets/",
    travelRule: { solution: "独自対応", note: "米FinCEN・カナダFINTRAC・リトアニアの3か国ライセンス取得。Notabene経由でTravel Rule対応済み" },
  },
  {
    id: "whitebit",
    name: "WhiteBIT",
    nameEn: "WhiteBIT",
    region: "overseas",
    country: "リトアニア（EU）",
    established: 2018,
    url: "https://whitebit.com/",
    domain: "whitebit.com",
    logoFile: "whitebit.png",
    logoColor: "#C9AA4C",
    description:
      "2018年設立、欧州最大級の取引所（トラフィック基準）。ユーザー数3,500万人超。リトアニアFCISほかEU10か国以上でVASP認可取得。CER.liveセキュリティ評価で業界トップ5。レンディング年率18%超のプランを提供。",
    tokens: [
      "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "AVAX",
      "LINK", "ATOM", "MATIC", "USDT", "USDC",
    ],
    features: ["spot", "futures", "staking", "lending", "api", "mobile_app", "credit_card"],
    fees: {
      exchangeMaker: 0.1,
      exchangeTaker: 0.1,
      roundTripCostPct: 0.20,
      spreadRating: "tight",
      depositFeeNote: "暗号資産入金無料",
      withdrawalFeeNote: "銘柄により異なる（WBTトークン保有で無料）",
    },
    maxLeverage: 100,
    tradingPairs: 780,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 4,
    notes:
      "Hacken.ioによるセキュリティ監査済み、CER.liveランキングトップ5。2024年に不正資金1億5,000万ドル超を阻止。WBTトークン保有者は手数料割引・出金無料などの特典あり。金融庁警告なし（2025年時点）。",
    affiliateUrl: "https://whitebit.com/referral/17705f2c-af98-405b-9a07-3d8570ed97ab",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録でボーナスあり",
    tokensUrl: "https://whitebit.com/trade/",
    travelRule: { solution: "独自対応", note: "2025年5月よりMiCA（EU暗号資産規制）準拠のTravel Rule本格実装。EU10か国以上のVASP認可保有" },
  },

  // ========== DEX ==========
  {
    id: "uniswap",
    name: "Uniswap",
    nameEn: "Uniswap",
    region: "dex",
    country: "分散型（米国法人）",
    established: 2018,
    url: "https://app.uniswap.org/",
    domain: "uniswap.org",
    logoFile: "uniswap.png",
    logoColor: "#FF007A",
    description:
      "Ethereum・L2最大のスポットDEX。AMM（自動マーケットメーカー）モデルの元祖。V4では「フック」機能でDEX初のカスタム流動性ロジックを実現。300万以上のトークンペアを取扱う。",
    tokens: ["BTC", "ETH", "USDC", "USDT", "LINK", "UNI", "AVAX", "MATIC", "SOL"],
    features: ["spot", "api"],
    fees: {
      exchangeTaker: 0.30,
      roundTripCostPct: 0.60,
      spreadRating: "moderate",
      depositFeeNote: "ウォレット接続のみ。法定通貨入金なし",
      withdrawalFeeNote: "ガス代のみ（Ethereum L1/L2により異なる）",
    },
    tradingPairs: 3000000,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 5,
    volumeRank: 2,
    notes:
      "DEXスポット市場の事実上の標準。V4（2024年）でカスタムフックを導入し拡張性が大幅向上。日本ユーザーの利用制限なし。UNIトークン保有者がプロトコルを統治。",
    affiliateUrl: "",
    affiliateType: "none",
    affiliateNote: "アフィリエイトプログラムなし",
    tokensUrl: "https://app.uniswap.org/explore/tokens",
    travelRule: { solution: "N/A",         note: "DEX（分散型）。KYC不要のため規制対象外" },
  },
  {
    id: "dydx",
    name: "dYdX",
    nameEn: "dYdX",
    region: "dex",
    country: "分散型（Cosmos appchain）",
    established: 2017,
    url: "https://dydx.exchange/",
    domain: "dydx.exchange",
    logoFile: "dydx.png",
    logoColor: "#6966FF",
    description:
      "独自Cosmosアプリチェーン上で動作するパーペチュアル特化DEX。180以上の取引ペアを提供。BTCなど主要ペアはMaker・Taker手数料ともにゼロ。取引リワードでDYDXトークンを獲得できる。",
    tokens: ["BTC", "ETH", "SOL", "DOGE", "AVAX", "LINK", "ATOM", "USDC"],
    features: ["futures", "api"],
    fees: {
      exchangeMaker: 0.00,
      exchangeTaker: 0.05,
      roundTripCostPct: 0.10,
      spreadRating: "tight",
      depositFeeNote: "USDC等でブリッジ入金。法定通貨入金なし",
      withdrawalFeeNote: "オンチェーン出金（ガス代相当）",
    },
    tradingPairs: 180,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 10,
    notes:
      "米国・カナダからのアクセスは不可。日本を含む多くの国は利用可能。BTC・SOLパーペチュアルは現在手数料ゼロ。取引量に応じてDYDXリワードあり。",
    affiliateUrl: "https://dydx.trade?ref=WideReefU7A",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから登録で特典あり",
    tokensUrl: "https://dydx.exchange/trade/BTC-USD",
    travelRule: { solution: "N/A",         note: "DEX（分散型）。KYC不要のため規制対象外" },
  },
  {
    id: "gmx",
    name: "GMX",
    nameEn: "GMX",
    region: "dex",
    country: "分散型（Arbitrum / Avalanche）",
    established: 2021,
    url: "https://app.gmx.io/",
    domain: "gmx.io",
    logoFile: "gmx.png",
    logoColor: "#2D42FC",
    description:
      "Arbitrum・Avalanche上のパーペチュアル特化DEX。GLP流動性プールモデルにより最大100倍レバレッジを実現。Chainlinkオラクルで板操作による強制清算を防止。GMX/GLPのステーキングで手数料収益（年利15-20%）を受け取れる。",
    tokens: ["BTC", "ETH", "AVAX", "LINK", "UNI", "USDC", "USDT"],
    features: ["futures", "staking", "api"],
    fees: {
      exchangeMaker: 0.05,
      exchangeTaker: 0.07,
      roundTripCostPct: 0.14,
      spreadRating: "tight",
      depositFeeNote: "ウォレット接続のみ。法定通貨入金なし",
      withdrawalFeeNote: "ガス代のみ（Arbitrum L2のため安価）",
    },
    maxLeverage: 100,
    tradingPairs: 40,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 15,
    notes:
      "GLPプールへの流動性提供で取引手数料収益の30%を受け取れる独自モデル。Chainlinkオラクル採用で価格操作リスクを低減。日本ユーザーの利用制限なし。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムあり",
    tokensUrl: "https://app.gmx.io/#/trade",
    travelRule: { solution: "N/A",         note: "DEX（分散型）。KYC不要のため規制対象外" },
  },
  {
    id: "jupiter",
    name: "Jupiter",
    nameEn: "Jupiter",
    region: "dex",
    country: "分散型（Solana）",
    established: 2021,
    url: "https://jup.ag/",
    domain: "jup.ag",
    logoFile: "jupiter.svg",
    logoColor: "#C7B53E",
    description:
      "Solana最大のDEXアグリゲーター。複数のDEXを横断して最安値ルートを自動探索。リミット注文・DCA（積立）・パーペチュアル取引も対応。JUPトークン保有者によるDAO統治。",
    tokens: ["BTC", "ETH", "SOL", "USDC", "USDT", "DOGE", "AVAX"],
    features: ["spot", "futures", "api"],
    fees: {
      exchangeTaker: 0.20,
      roundTripCostPct: 0.40,
      spreadRating: "tight",
      depositFeeNote: "ウォレット接続のみ。Solanaアドレスが必要",
      withdrawalFeeNote: "Solanaガス代のみ（非常に安価）",
    },
    tradingPairs: 500,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 3,
    notes:
      "Solana上のスワップ取引の過半数を処理。複数DEXを束ねたルーティングで最良レートを自動実現。パーペチュアル取引ではJLP流動性プールへのステーキングで収益を獲得可能。",
    affiliateUrl: "https://jup.ag/?ref=en3l92v6ozb7",
    affiliateType: "referral",
    affiliateNote: "紹介リンクから利用で手数料の一部が還元",
    tokensUrl: "https://jup.ag/tokens",
    travelRule: { solution: "N/A",         note: "DEX（分散型）。KYC不要のため規制対象外" },
  },
  {
    id: "hyperliquid",
    name: "Hyperliquid",
    nameEn: "Hyperliquid",
    region: "dex",
    country: "分散型（Cayman Islands）",
    established: 2023,
    url: "https://app.hyperliquid.xyz/",
    domain: "hyperliquid.xyz",
    logoFile: "hyperliquid.png",
    logoColor: "#00D4AA",
    description:
      "独自Layer-1上で動作する完全オンチェーンのDEX。DEXパーペチュアル市場シェア70%超を誇り、CEX並みの速度（サブ秒確定）と流動性を実現。KYC不要でウォレット接続のみで取引可能。",
    tokens: ["BTC", "ETH", "SOL", "DOGE", "AVAX", "LINK", "ATOM", "USDC"],
    features: ["futures", "spot", "staking", "api"],
    fees: {
      exchangeMaker: -0.01,
      exchangeTaker: 0.035,
      roundTripCostPct: 0.05,
      spreadRating: "tight",
      depositFeeNote: "USDC/USDT等でブリッジ入金。法定通貨入金なし",
      withdrawalFeeNote: "オンチェーン出金（ガス代のみ）",
    },
    tradingPairs: 150,
    japaneseSupport: false,
    fsaRegistered: false,
    trustScore: 4,
    volumeRank: 5,
    notes:
      "DEXパーペチュアル市場シェア70%超。2024年11月のHYPEトークン配布（全流通量の31%をユーザーに無償配布）が話題を呼んだ。日本ユーザーのアクセス制限なし。KYC不要。",
    affiliateUrl: "",
    affiliateType: "referral",
    affiliateNote: "公式リファラルプログラムで発行可能",
    tokensUrl: "https://app.hyperliquid.xyz/trade",
    travelRule: { solution: "N/A",         note: "DEX（分散型）。KYC不要のため規制対象外" },
  },
];

export const FEATURE_LABELS: Record<string, string> = {
  spot: "現物取引",
  futures: "先物取引",
  margin: "証拠金取引",
  staking: "ステーキング",
  lending: "レンディング",
  nft: "NFT",
  copy_trading: "コピートレード",
  options: "オプション取引",
  demo: "デモ取引",
  api: "API提供",
  mobile_app: "スマホアプリ",
  japan_yen_deposit: "日本円入金",
  credit_card: "クレジットカード",
};

export const ALL_TOKENS: string[] = [
  // メジャー
  "BTC", "ETH", "XRP", "SOL", "ADA", "DOT", "DOGE", "LTC", "BCH", "AVAX",
  "LINK", "ATOM", "FIL", "BNB",
  // DeFi / ステーブル
  "USDT", "USDC", "DAI", "MKR", "UNI", "GRT",
  // Layer2 / インフラ
  "MATIC", "POL", "ARB", "OP", "IMX", "ASTR", "OAS", "HBAR", "ALGO",
  "XTZ", "FLR", "NEAR", "APT", "TRX",
  // NFT / ゲーム / ミーム
  "AXS", "SAND", "MANA", "ENJ", "CHZ", "APE", "SHIB", "PEPE", "RNDR", "MASK",
  // 国内主要
  "XLM", "XEM", "XYM", "MONA", "LSK", "IOST", "QTUM", "BAT", "ETC", "OMG",
  "FLARE", "ZPG",
];
